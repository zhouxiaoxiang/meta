...
(defn- source-query-cards
  ...
  (when-let [ids-of-dbs-that-support-source-queries (not-empty (ids-of-dbs-that-support-source-queries))]
    (transduce
     (comp (filter card-can-be-used-as-source-query?)
           xform)
     (completing conj #(hydrate % :collection))
     []
     (db/select-reducible Card {:select   [:name :description :database_id :dataset_query :id :collection_id :result_metadata
                                           [{:select   [:status]
    ...
    (db/select [Card :id :dataset :database_id :name :collection_id [:collection.name :collection_name]]
               {:where    [:and
                           ...
                           (cond
                             ...
                             [:like (h2x/cast (if (= (mdb.connection/db-type) :mysql) :char :text) :report_card.id) (str search-id "%")]
