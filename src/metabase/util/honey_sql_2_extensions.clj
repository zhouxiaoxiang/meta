...
;;; TODO -- consider whether we should namespace these to make our code more obvious
...
(defn- format-distinct-count [_fn [expr]]
  (let [[sql & args] (sql/format-expr expr)]
...
(defn- format-percentile-cont [_fn [expr p]]
  (let [[sql & args] (sql/format-expr expr)]
    (into [(str "PERCENTILE_CONT(" sql ") within group (order by "  ")")]
          args)))
...
(s/defn cast :- (s/pred typed? "::typed expr")
...
(s/defn quoted-cast :- (s/pred typed? "::typed expr")
...
(s/defn maybe-cast :- (s/pred typed? "::typed expr")
