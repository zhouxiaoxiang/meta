(ns metabase.db.fix-mysql-utf8-test
  (:require
   ...
   [toucan.db :as db]
   [toucan2.core :as t2]
   [toucan2.map-backend.honeysql2 :as t2.sql]))
...
(defn- insert-row! []
  (db/query [(str "INSERT INTO metabase_database (engine, name, created_at, updated_at, details) "
        ...
        (let [data-source (test-data-source)
          ...
          (testing (format "Migrating %s charset -> utf8mb4\n" charset)
            ...
            (t2/with-connection [_conn jdbc-spec]
              (binding [t2.sql/*options* (assoc t2.sql/*options* :dialect :mysql)]
